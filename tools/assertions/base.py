...
@allure.step("Check that {name} equals True")
def assert_is_true(actual: Any, name: str):
    ...
    """
    logger.info(f"Check that '{name}' equals True")
    assert actual, (
        f"Incorrect value {name}. "
        f"Expected true value but got: {actual}"
    )

def assert_length(actual: Sized, expected: Sized, name: str): #Sized это тип данных у которых может быть измеренна длинна
    """
    ...
    with allure.step(f"Check that lenght of {name} equals to {len(expected)}"):
        logger.info(f"Check that lenght of '{name}' equals to {len(expected)}")
